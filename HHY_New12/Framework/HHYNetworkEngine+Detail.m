...
-(void)queryCheckOrderDetail:(NSString *)orderNum username:(NSString *)username block:(HHYResponseBlock)block
{
     

    NSString *path = [[self path:@"order/checkOrderDetail" needToken:YES] stringByAppendingFormat:@"/%@/%@", orderNum,username];
    [self postURLDataForPath:path params:nil block:^(NSError *error, id data) {
        block(error,data);
    }];
}
...
-(void)orderUpdate:(NSDictionary *)dict block:(HHYResponseBlock)block
{
    NSString *path = [NSString stringWithFormat:@"order/stateUpdate/%@/%@/%@",[self getToken],[dict objectForKey:@"orderNo"],[dict objectForKey:@"sid"]];
    [self postURLDataForPath:path params:dict block:^(NSError *error, id data) {
        block(error,data);
    }];
}
...
- (void)getHotelPayURL:(NSString *)orderNum block:(HHYResponseBlock)block;
{
   
    NSString *path = [NSString stringWithFormat:@"alipay/getParamByOrderNo.jspx?orderNo=%@",orderNum];
    [self getHotelDataForPath:path params:nil block:^(NSError *error, id data) {
        NSLog(@"%@",[data responseString]);
        block(error,[data responseString]);
    }];
}
//查询审批人
...
- (void)searchAirOrderNeedApply:(NSString *)email   block:(HHYResponseBlock)block
{
    ...
    ASIFormDataRequest *request = [ASIFormDataRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"http://113.11.197.184:7070/cloud/examine/queryOrder/%@?email=%@",[self getToken],email]]];
    [request setTimeOutSeconds:200];
    [request startAsynchronous];
    __weak typeof (request) blockrequest = request;
    [request setCompletionBlock:^{
        block(nil,[blockrequest responseString]);
    }];

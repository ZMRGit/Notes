...
-(void)countTotalAmount:(NSNotification *)notifition
{
    ...
    totalCount = servicePrice + roomPrice * [_roomNum integerValue];
}
...
//            [[HotelOrderParameter shareOrderParameter].orderDic setObject:EnCodingNSString([[NSUserDefaults standardUserDefaults] objectForKey:@"address"]) forKey:@"address"];
   ...
   else if(([[HotelOrderParameter shareOrderParameter].orderDic objectForKey:@"postCode"] == nil || ((NSString *)[[HotelOrderParameter shareOrderParameter].orderDic objectForKey:@"postCode"]).length == 0) && cell.isBut.selected)
    {
    ...
    else if(([[HotelOrderParameter shareOrderParameter].orderDic objectForKey:@"address"] == nil || ((NSString *)[[HotelOrderParameter shareOrderParameter].orderDic objectForKey:@"address"]).length == 0) && cell.isBut.selected)
    {
...
-(void)textFieldDidEndEditing:(UITextField *)textField
{
    ...
    if(cell)
    {
        if(cell.postcode.text)
        {
            [[HotelOrderParameter shareOrderParameter].orderDic setObject:cell.postcode.text forKey:@"postCode"];
        }
        if(cell.adress.text)
        {
            [[HotelOrderParameter shareOrderParameter].orderDic setObject:cell.adress.text forKey:@"address"];
        }
    }
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (alertView.tag==131) {
        ...
        hopvc.price = [NSString stringWithFormat:@"%.0f",totalCount];

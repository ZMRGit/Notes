...
- (void)getLoadData
{
    ...
    [[HHYNetworkEngine sharedInstance] HotelOrderDetail:_orderNum block:^(NSError *error, id data) {
        ...
        if (!error) {
            ...
            hhyPrice = [NSString stringWithFormat:@"%@",[dic objectForKey:@"hhyAmount"]];
...
- (void)creatScreen
{
    ...
    _hotelTable.backgroundColor = kADWColor(235, 235, 235, 1);
    _hotelTable.separatorStyle = UITableViewCellAccessoryNone;
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section==0) {
        ...
        if (indexPath.row==0) {
            ...
            NSArray *serviceList = [[roomList objectAtIndex:indexPath.row - 1] objectForKey:@"serviceList"];
        
            fl = 53 + 30 * serviceList.count;
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section==0) {
        ...
        if (indexPath.row==0) {
            ...
            UIImageView *line  = [[UIImageView alloc]initWithFrame:CGRectMake(20, lbRoomType.frame.origin.y + lbRoomType.frame.size.height + 5, 300, 1)];
            line.backgroundColor = kADWColor(225, 225, 225, 1);
            if ([roomList count]!=0) {
...
//            UILabel *lbEndDate = (UILabel *)[cell viewWithTag:1002];
            ...
            inMonth.lineBreakMode = NSLineBreakByCharWrapping;
            UILabel *inDay = (UILabel *)[cell viewWithTag:1101];
            UILabel *outMonth = (UILabel *)[cell viewWithTag:1200];
            outMonth.lineBreakMode = NSLineBreakByCharWrapping;
            ...
            */
            //table 刷新
            NSLog(@"%@",[roomList objectAtIndex:0]);
            if ([roomList count]!=0) {
                ...
                {
                    NSDictionary *d = [serviceList objectAtIndex:i];
                    UILabel *serviceName = [[UILabel alloc]initWithFrame:CGRectMake(20, 53 + 30 * i, 150, 30)];
                    serviceName.font = [UIFont systemFontOfSize:14];
                    serviceName.textColor = kADWColor(80, 80, 80, 1);
                    serviceName.text = [d objectForKey:@"serviceName"];
                    [cell addSubview:serviceName];
                    UILabel *fen = [[UILabel alloc]initWithFrame:CGRectMake(170, 53 + 30 * i, 60, 30)];
                     fen.font = [UIFont systemFontOfSize:14];
                    fen.textAlignment = NSTextAlignmentRight;
                    fen.textColor = kADWColor(80, 80, 80, 1);
                    fen.text = [NSString stringWithFormat:@"%@ 份",[d objectForKey:@"qty"]];
                    [cell addSubview:fen];
                    
                    
                    UILabel *night = [[UILabel alloc]initWithFrame:CGRectMake(240, 53 + 30 * i, 60, 30)];
                     night.font = [UIFont systemFontOfSize:14];
                    night.textAlignment = NSTextAlignmentRight;
                    night.textColor = kADWColor(80, 80, 80, 1);
                    night.text = [NSString stringWithFormat:@"%@ 晚",[d objectForKey:@"night"]];
...
//        cell.emainTF.enabled = NO;
    ...
    if(indexPath.row == rowNumber - 1)
    {
        ...
        {
            CustonLine *line2 = [[CustonLine alloc]initWithFrame:CGRectMake(0, 53+30 * serviceCount - 1, 320, 1)];

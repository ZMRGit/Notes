        ...
        if (!error) {
            if ([data isKindOfClass:[NSDictionary class]]) {
                ...
                if(tuiPrice > 0)
                {
                    ...
                    [but setTitle:@"同意" forState:UIControlStateNormal];
                }
                else if(tuiPrice < 0)
                {
                    UIButton *but = (UIButton *)[self.view viewWithTag:2000];
...
- (void)creatBottom
{
    ...
    UIButton *rejectBut = [UIButton buttonWithType:UIButtonTypeCustom];
    [rejectBut setTitle:@"拒绝" forState:UIControlStateNormal];
    [rejectBut setBackgroundImage:[UIImage imageNamed:@"tuiding"] forState:UIControlStateNormal];
    [rejectBut setBackgroundImage:[UIImage imageNamed:@"tuidingBTN"] forState:UIControlStateHighlighted];
    [rejectBut addTarget:self action:@selector(rejectClick:) forControlEvents:UIControlEventTouchUpInside];
    rejectBut.tag = 2002;
    rejectBut.frame = CGRectMake(200, 5, 50, 40);
    
    [view addSubview:rejectBut];
...
- (void)btnClick:(UIButton *)sender
{
    ...
    [dic setObject:self.airModel.orderNumber forKey:@"orderNo"];
    [dic setObject:@"20" forKey:@"sid"];
    [[HHYNetworkEngine sharedInstance] orderUpdate:dic block:^(NSError *error, id data) {
        if(!error)
        {
            [Utils alertWithTitle:[NSString stringWithFormat:@"%@",data] message:nil];
        }
    }];
...
-(void)rejectClick:(UIButton *)but
{
    if(but.tag == 2002)
    {
        NSMutableDictionary *dic = [NSMutableDictionary dictionary];
        [dic setValue:self.airModel.orderNumber forKey:@"orderNo"];
        [dic setValue:@"16" forKey:@"sid"];
        [[HHYNetworkEngine sharedInstance] orderUpdate:dic block:^(NSError *error, id data) {
            if(!error)
            {

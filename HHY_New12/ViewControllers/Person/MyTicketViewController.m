...
- (void)viewDidLoad
{
    ...
    airStateArray = @[@"预订中",@"待收款",@"已处理",@"记录已消除",@"逻辑删除",@"已确认",@"已审批",@"审批中",@"审批拒绝",@"导入",@"出票提示邮件",@"旅客行程邮件",@"已付款",@"已结算",@"已出票",@"已退票",@"已改期",@"改期申请",@"退票申请",@"航班延误申请",@"退票已核算",@"已退款"];
    airNumState = @[@"0",@"1",@"2",@"3",@"33",@"5",@"6",@"4",@"14",@"7",@"71",@"72",@"88",@"8",@"9",@"20",@"24",@"21",@"22",@"23",@"221",@"19"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (tableView==_tableView) {
        ...
        else if ([model.ticketState isEqualToString:@"21"])
        {
            ChangeDateSQViewController *chvc = [[ChangeDateSQViewController alloc] init];
            chvc.orderNum = model.orderNumber;
            chvc.allCountPrice = model.total;
  
            [self.navigationController pushViewController:chvc animated:YES];
        }
        else if ([model.ticketState isEqualToString:@"22"])
        {
            AirReturnTicketSQViewController *vc = [[AirReturnTicketSQViewController alloc]init];
            vc.orderNum = model.orderNumber;
            vc.allCountPrice = model.total;

...
-(void)responseVoipManagerStatus:(ECallStatusResult)event callID:(NSString*)callid data:(NSString *)data
{
    ...
    switch (event)
    {
        ...
        case ECallStatus_Failed:
        {
            ...
            {
                self.realTimeStatusLabel.text = @"未上线仅限呼叫已配置测试号码";
            }
            ...
            {
                [NSTimer scheduledTimerWithTimeInterval:3.0f target:self selector:@selector(releaseCall) userInfo:nil repeats:NO];
            }
            else
            {

...
- (void)onCallBackWithReason:(CloopenReason *)reason andFrom:(NSString *)src To:(NSString *)dest
{
    ...
    {
        callReason = [NSString stringWithFormat:@"错误%d,%@",reason.reason, reason.msg];
...
- (void)onIncomingCallReceived:(NSString*)callid withCallerAccount:(NSString *)caller withCallerPhone:(NSString *)callerphone withCallerName:(NSString *)callername withCallType:(NSInteger)calltype
{
    ...
    {
        [self.VoipCallService rejectCall:callid andReason:EReasonBusy];
...
{
    if ([self.rejectCallId isEqualToString:callid])//拒绝呼叫的不用上报消息
    {
        return;
    }
...
#pragma mark - 聊天室相关函数
//创建聊天室
- (void) startChatroomWithName:(NSString *)roomName andPassword:(NSString *)roomPwd andSquare:(NSInteger)square andKeywords:(NSString *)keywords inAppId:(NSString*)appid andIsAutoClose:(BOOL)isAutoClose
{
    [self.VoipCallService startChatroomInAppId:appid withName:roomName andSquare:square andKeywords:keywords andPassword:roomPwd andIsAutoClose:isAutoClose];
}
 ...
 */
- (void) startVideoConferenceInAppId:(NSString*)appId withName:(NSString *)conferenceName andSquare:(NSInteger)square andKeywords:(NSString *)keywords andPassword:(NSString *)conferencePwd andIsAutoClose:(BOOL)isAutoClose
{
    [self.VoipCallService startVideoConferenceInAppId:appId withName:conferenceName andSquare:square andKeywords:keywords andPassword:conferencePwd andIsAutoClose:isAutoClose];
}

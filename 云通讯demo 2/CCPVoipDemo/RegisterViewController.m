...
#pragma mark - 由setModalEngineDelegate设置返回的注册消息
- (void)responseVoipRegister:(ERegisterResult)event data:(NSString *)data
{
    if (event == ERegisterSuccess)
    {
        ...
        NSString* strVersion = [self.modelEngineVoip getSDKVersion];
        NSLog(@"%@",strVersion);
    }
    ...
    {
        [self dismissProgressingView];
        [self  popPromptViewWithMsg:@"注册失败，请稍后重试！" AndFrame:CGRectMake(0, 160, 320, 30)];
    }
